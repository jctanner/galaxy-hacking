...
class BenchMarker:
    ...
    sync_collections_log = 'logs/sync_collections.log'
    benchmark_log = 'logs/benchmark.log'

    def __init__(self):

        logdir = os.path.dirname(self.benchmark_log)
        ...
        self.UPSTREAM_BASEURL = 'http://192.168.1.20:8080'
        ...
        while next_url:
            ...
            if 'next_link' in ds:
                if ds.get('next_link') is None:
                    break
                next_url = self.UPSTREAM_BASEURL + ds['next_link']
            else:
    ...
    def get_or_create_remote(self, name, remote_url, requirements):
        ...
        if name in rmap:
            ...
            while True:
                trr = requests.get(task_url, auth=self.auth)
                tresp = trr.json()
                if tresp['state'] in ['completed', 'failed']:
                    break
    ...
    def run_benchmarks(self):
        ...
        baseurls = {
            ...
            'X_REPO_SEARCH': '/pulp_ansible/galaxy/default/api/v3/plugin/ansible/search/collection-versions/',
        }
    ...
    def incremental_sync(self):
        ...
        for col in self.iterate_upstream_collections(self.UPSTREAM_BASEURL + '/api/v2/collections/'):

            # skip if already synced
            col_fqn = [col['namespace']['name'], col['name']]
