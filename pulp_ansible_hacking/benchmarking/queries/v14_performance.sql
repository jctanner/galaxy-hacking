...
GROUP BY
    cr.pulp_id, acv.content_ptr_id, core_content.pulp_id, rcvs.is_signed, rcd.is_deprecated
HAVING
   MAX(crv1.number) > coalesce(MAX(crv2.number), -1)
;
...
--SELECT reponame,namespace,name,version,is_signed,is_deprecated FROM repository_collection_version_2 WHERE name='panther' limit 10;

SELECT reponame,namespace,name,version FROM repository_collection_version_2;
