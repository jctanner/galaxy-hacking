...
CACHED_SESSION = requests_cache.CachedSession('galaxy_cache')
...
def safe_fetch(url, use_cache=False):
    rr = None
    counter = 0
    while True:
        counter += 1
...
def get_namespace_owners_details(baseurl, ns_id, use_cache=False):
    # get the owners too
    owners = []
    next_owners_url = baseurl + f'/api/v1/namespaces/{ns_id}/owners/'
    while next_owners_url:
        logger.info(f'fetch {next_owners_url}')
        o_data = safe_fetch(next_owners_url, use_cache=use_cache).json()
        for owner in o_data['results']:
            owners.append(owner)
        if not o_data.get('next'):
            break
...
def upstream_namespace_iterator(
    ...
    while next_url:
        ...
        page = safe_fetch(next_url, use_cache=use_cache)
        ...
        for ndata in ds['results']:
            ...
            if skip_names and ndata['name'] in skip_names:
                continue
            #import epdb; epdb.st()
            ...
            # get the owners too
            ndata['summary_fields']['owners'] = \
                get_namespace_owners_details(_baseurl, ns_id, use_cache=use_cache)

            # send the collection
            namespace_count += 1
            yield pagenum, total, ndata
